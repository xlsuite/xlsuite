class CreatingListingEmbedSnippetConfigurations < ActiveRecord::Migration
  def self.up
    config = nil
    ...
    Account.all(:select => "id").map(&:id).each do |account_id|
      config_group_name = "Listings"
      config_description = "The title of the snippet that is used as a template to generate embed code of a listing"
      config_name = "listing_embed_code_snippet"
  ...
  def self.down
    Configuration.delete_all(:name => "listing_embed_code_snippet")
  end
end
