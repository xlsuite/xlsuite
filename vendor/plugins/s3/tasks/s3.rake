...
namespace :s3 do
  ...
  namespace :backup do
    ...
    task :db  do
      ...
      msg "dumping db"
      cmd = "mysqldump --opt --skip-add-locks --single-transaction -u#{user} "
  ...
  def make_bucket(name)
    response = conn.create_bucket(bucket_name(name)).http_response
    msg = response.message
    raise "Could not make bucket #{bucket_name(name)}.  Msg: #{msg}, response: #{response.to_yaml}" if msg != 'OK'
