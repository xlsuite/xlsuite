      ...
      module S3Backend
        ...
        def self.included(base) #:nodoc:
          ...
          begin
            ...
            raise "Missing config/amazon_s3.yml configuration file" unless File.file?(@@s3_config_path)
            @@s3_config = YAML.load(ERB.new(File.read(@@s3_config_path)).result)
            raise "Missing #{RAILS_ENV} section in config/amazon_s3.yml" unless @@s3_config.has_key?(RAILS_ENV)
            @@s3_config = @@s3_config[RAILS_ENV].symbolize_keys
          rescue
