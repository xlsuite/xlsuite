    ...
    module ActionController # :nodoc:
      ...
      module InstanceMethods # :nodoc:
        private
          def dispatch_web_service_request
            if request.get?
              render(:text => 'GET not supported', :status => '500 GET not supported')
              return
            end
            exception = nil
            begin
          ...
          def send_web_service_error_response(ws_request, exception)
            if ws_request
              ...
              render(:text => "Internal protocol error: #{message}#{backtrace}", :status => "500 Internal Protocol Error")
            end
          end
      ...
      module WsdlAction # :nodoc:
        ...
        def wsdl
          ...
          when :post
            render(:text => 'POST not supported', :status => '500 POST not supported')
          end
        end
