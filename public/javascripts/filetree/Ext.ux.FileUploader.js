...
Ext.extend(Ext.ux.FileUploader, Ext.util.Observable, {
	 ...
	 */
	,createForm:function(record, params) {
      ...
      ,target:'file-upload-iframe'
			...
			,cn:[{
				...
				,type:'hidden'
				,name:'zip_file'
				,value:Ext.get('file_tree_panel_unzip_checkbox').dom.value
			},{
				 tag:'input'
				,type:'hidden'
				,name:'tag_list'
				,value:Ext.get('file_tree_panel_tag_list_field').dom.value
			}]
		});
		if(record) {
...
//END
	...
	,uploadFile:function(record, params) {
		...
		var form = this.createForm(record, params);
