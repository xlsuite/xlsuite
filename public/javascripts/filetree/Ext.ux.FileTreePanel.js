...
/**
	...
	,initComponent:function() {
      ...
      /**
			 * @event nodenameclick
			 * Fires after the nodename has been clicked
			 * @param {Ext.ux.FileTreePanel} this
			 * @param {Ext.tree.AsyncTreeNode} new node/directory that has been created
			 */
			,'nodenameclick'
...
//				,menu:this.getContextMenu()
				...
				switch(options.params.cmd) {
					case 'delete':
						if(true !== this.eventsSuspended) {
							this.fireEvent('delete', this, this.getPath(options.node), options.node.parentNode);
						}
	...
	,confirmOverwrite:function(filename, callback, scope) {
      ...
      Ext.Msg.alert("Failure", String.format("Folder {0} already exists", filename) + '.');
    }
	}
	// }}}
	// {{{
  ...
  ,getUploadPanel:function() {
	...
	,onContextClick:function(menu, item, e) {
      ...
      case 'nodename':
        this.fireEvent('nodenameclick', this, node);
      break;
			default:
			break;
		}
	...
	,showContextMenu:function(node) {
		...
		// set node name
		menu.getItemByCmd('nodename').setText(Ext.util.Format.ellipsis("Edit " + node.text, 22));
		...
		menu.setItemDisabled('nodename', node === this.root);
