...
Ext.ux.FileTreeMenu = function(config) {
    ...
    ,{
      text: 'Add Tags on Upload',
      menu: {
        items: [new Ext.menu.Adapter(new Ext.form.TextField({
          id: 'file_tree_panel_tags_textfield',
          listeners: {
            'change': function(me, newValue, oldValue){
              Ext.get('file_tree_panel_tag_list_field').dom.value = newValue;
            },
            'focus': function(me){
              me.el.up('li').removeClass('x-menu-list-item');
            }
          }
        }), {
          hideOnClick: false
    ...
    ,new Ext.menu.CheckItem({
      text: 'Unzip on Upload?',
      hideOnClick: false,
      checkHandler: function(me, checked){
        if(checked){
          Ext.get('file_tree_panel_unzip_checkbox').dom.value = 1;
        }
        else{
          Ext.get('file_tree_panel_unzip_checkbox').dom.value = 0;
        }
      }
    })
